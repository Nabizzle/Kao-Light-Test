...
* [Description of Each Script](#description-of-each-script)
  ...
  * Arduino Uno PCB Shield
  ...
  Plug in the panning servo to signal pin 10 and the tilting servo to signal pin 11. **Note on older PCB's that in the image of the PCB these are labelled as servo 2 and servo 1 respectively, but in the current PCB, these servos are labelled as what angle they change. If the PCB is reprinted, then this note should be deleted.** Make sure that the 7.8 V LiPo battery is also plugged in and check that the laser is not on automatically. If it is, you may need to adjust the shield or check that the laser connector is not touching metal. Plug in the USB cable to the Arduino to power it.
